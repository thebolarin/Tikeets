...
let prefix = process.env.TICKEET_REDIS_KEY_PREFIX;
 ...
 * value for that ID.
 *
 * Key name: prefix:tickeet:info:[siteId]
 ...
 */
const getHashKey = (id) => getKey(`tickeet:info:${id}`);

/**
 * Returns the Redis key name used for the set storing all IDs.
 *
 * Key name: prefix:tickeet:ids
 * Redis type stored at this key: set
 *
 * @returns - the Redis key name used for the set storing all IDs.
 */
const getIDsKey = () => getKey('tickeet:ids');
